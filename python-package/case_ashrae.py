...
def get_leak_():
    ...
    print("====== get_leak_@{} ......".format(pkl_path))
...
for fold, (train_index, valid_index) in enumerate(kf.split(df_train, Y_train)):
    X_train, X_valid = df_train[all_features].loc[train_index], df_train[all_features].loc[valid_index]
    y_train, y_valid = Y_train.iloc[train_index], Y_train.iloc[valid_index]
    if fold==0:
        print(f"X_train={X_train.shape} cols={X_train.columns}")
        print(f"X_valid={X_valid.shape} cols={X_valid.columns}")
